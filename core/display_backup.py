...
class Display:
    ...
    def _mock_display(self, lines: List[str]) -> None:
        ...
        """
        # Clear previous display and show header
        print("\n" + "=" * 40)
        print("OLED DISPLAY UPDATE:")
        print("=" * 40)
    ...
    def show_todo_menu(self) -> bool:
        """Show todo menu with instructions."""
        lines = [
            "Todo Commands:",
            "Say 'add' to create",
            "Say 'list' to view",
    ...
    def show_device_list(self, devices: List[Dict[str, str]], cursor: int = 0) -> bool:
        """
        Show device list with cursor and numbered options.
        ...
        for i, device in enumerate(visible_devices):
            device_index = cursor + i
            is_current = (device_index == cursor)  # Current cursor position
            prefix = "> " if is_current else "  "
            number = device_index + 1  # 1-based numbering
            name = device.get('name', 'Unknown')[:14]  # Truncate to fit with number
